...
@pytest.fixture(name="kafka_servers")
def fixture_kafka_server(request, session_tmppath: Path) -> Iterator[KafkaServers]:
    ...
    kafka_dir = session_tmppath / new_random_name('kafka')
    zk_dir = session_tmppath / new_random_name('zk')
    transfer_file = session_tmppath / new_random_name('config')
