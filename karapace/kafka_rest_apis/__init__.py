...
class KafkaRest(KarapaceBase):
    ...
    def _add_kafka_rest_routes(self) -> None:
        # Brokers
        self.route(
            "/brokers", callback=self.list_brokers, method="GET", rest_request=True, with_request=True, json_body=False
        )
        ...
        # Topics
        self.route("/topics", callback=self.list_topics, method="GET", rest_request=True, with_request=True, json_body=False)
        self.route(
            "/topics/<topic:path>",
    ...
    def get_user_proxy(self, request: HTTPRequest) -> "UserRestProxy":
        log.info("get user proxy for %s", request)
        key = ""
        if self.config.get("rest_authorization", False):
            log.info("Using rest authorization")
            auth_header = request.headers.get("Authorization")
            log.info("Using rest auth %s", auth_header)
    ...
    async def produce_messages(self, *, topic: str, prepared_records: List) -> List:
        ...
        for result in await asyncio.gather(*send_results, return_exceptions=True):
            ...
            # Exceptions below are raised before data is sent to Kafka
                ...
                log.exception("Timed out waiting for publisher buffer")
            ...
            # Exceptions below are raised after data is sent to Kafka
                ...
                log.exception("Async task cancelled")
                # cancel is retriable
